    ...
    <table-list :data-header="dataHeader" :url="tabUrl" :params="page" @get-table-data="getTableData"
      ...
      <template slot-scope="scope" slot="handle">
        <!--<div v-if="scope.row.state == 0">-->
          ...
          <!--<span class="handle"  style="margin-left: 10px;" @click="writePriceMark(scope.row.id)">填写进货价</span>-->
          <a class="handle" style="margin-left: 5px;" :href="exportQuotation + scope.row.id">导出</a>
          <!--<span class="handle" @click="viewDetail(scope.row.id)">查看</span>-->
          <router-link class="handle" :to="{name: 'purchaseDetail', params:{id: scope.row.id}}" tag="a" target="_blank">查看</router-link>
       <!-- </div>-->
        <!--<div v-else>
          <span class="handle" @click="viewDetail(scope.row.id)">查看</span>
...
<script>
  export default {
    data () {
      return {
        ...
        dataHeader: [
          ...
          { prop: 'handle', label: '操作', width: '250', slot: true }
        ],
        tablePerformance: 'purchase_table',
        searchData: [{
          key: 'startTime',
          label: '创建时间',
          value: '',
    ...
    methods: {
      ...
      purchaseSearch (id) {
        this.$router.push({ name: 'purchaseSearch', params: { id: id } })
      },
      writePriceMark (id) {
        this.$router.push({ name: 'purchaseRemark', params: { id: id } })
      },
      viewDetail (id) {
