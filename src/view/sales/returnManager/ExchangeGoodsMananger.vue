  ...
  export default {
    data () {
      return {
        ...
        goodsListTable: 'tab_exgoodsTable_list',
        params: {returnTypeCode: 2},
        returnTypeCode: 2,
        goodsDataHeader: [
    ...
    methods: {
      ...
      onview (row) {
        let routeData = this.$router.resolve({
          ...
          params: {state: 'exchange', id: row.id, type: this.type}
        })
        window.open(routeData.href, '_blank')
//        this.$router.push({name: this.routerName(), params: {state: 'exchange', id: row.id, type: this.type}})
      },
      onaudit (row) {
        let routeData = this.$router.resolve({
          name: this.routerName(),
