  ...
  <div>
    <el-tabs>
      <el-tabs v-model="activeName" type="card" @tab-click="tabClick">
        <el-tab-pane :label="'合同物资--待审批(' + (countData.financeDeptCount ? countData.financeDeptCount : 0) + ')'"
                     name="first">
        </el-tab-pane>
        <el-tab-pane :label="'常规物资--待审批(' + (countData.routineCount ? countData.routineCount : 0) + ')'"
                     name="two">
        </el-tab-pane>
        <el-tab-pane :label="'外审驳回--待审批(' + (countData.externalCount ? countData.externalCount : 0) + ')'"
                     name="three">
        </el-tab-pane>
        <el-tab-pane :label="'采购驳回记录(' + (countDataElse.rejectPlanCount ? countDataElse.rejectPlanCount : 0) + ')'"
                     name="four">
        </el-tab-pane>
        <el-tab-pane label="采购通过记录" name="five">
        </el-tab-pane>
        <el-tab-pane
          :label="'销售驳回记录(' + (countDataElse.rejectDeliveryCount ? countDataElse.rejectDeliveryCount : 0) + ')'"
          name="six">
        </el-tab-pane>
        <el-tab-pane label="销售通过记录" name="seven">
        </el-tab-pane>
      </el-tabs>
    </el-tabs>
    <v-search :data="searchDataInfo" @on-click="search"></v-search>
    <tab-but v-if="indexCode === '0' || indexCode === '1' || indexCode === '2'" :data="btnData"
    ...
    <table-list :data-header="dataHeaderData" :url="tabUrl" :params="page" @get-table-data="getTableData"
      ...
      <template v-slot:handle="scope" class='button-class' >
        ...
        <a v-if="indexCode === '5' || indexCode === '6'" class="handle"
           @click="toDetail(scope.row.goodsName, '1')">历史价</a>
        <a v-if="indexCode === '0' || indexCode === '1'" class="handle"
           @click="toReject(scope.row, '2')">驳回采购</a>
        <a v-if="indexCode === '1' || indexCode === '2'" class="handle" @click="toReject(scope.row, '1')">驳回销售</a>
      </template>
    </table-list>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        dataHeader: [
          ...
          {prop: 'grossMarginRatio', label: '毛利率%', slot: true},
        ...
        tabUrl1: this.$Api.examine.findFinanceDeptMaterialPagerByNode,
        tabUrl2: this.$Api.examine.findRoutineMaterialPagerByNode,
        tabUrl3: this.$Api.examine.findExternalMaterialPagerByNode,
        tabUrl4: this.$Api.examine.findAuditRecordGoodsPagerByNode,
        urlCount: this.$Api.examine.findFinanceDeptMaterialCount,
        urlCountElse: this.$Api.examine.findListPurchaseMaterialCountByNode,
        page: {
          totalNum: 0,
          currentPage: 1,
          offset: 0,
    ...
    computed: {
      tabUrl () {
        let url = ''
        if (this.indexCode === '3' || this.indexCode === '4' || this.indexCode === '5' || this.indexCode === '6') {
          url = this.tabUrl4
        } else if (this.indexCode === '0') {
          url = this.tabUrl1
        } else if (this.indexCode === '1') {
          url = this.tabUrl2
        } else if (this.indexCode === '2') {
          url = this.tabUrl3
        }
        return url
      }
    ...
    created () {
      ...
      this.searchDataInfo = this.searchData
    },
    methods: {
      toPass () {
        ...
        if (this.indexCode === '0') {
          ...
          url = this.$Api.examine.routineBatchInternalAuditPass
        } else {
          url = this.$Api.examine.batchInternalAuditPass
        }
        if (arrData && arrData.length > 0) {
          if (arrData.length > 50) {
            this.$message('批量通过数量不能超过50')
          } else {
      ...
      toDismissal (res, type) {
        ...
        if (this.indexCode === '0' || this.indexCode === '1' || this.indexCode === '2') {
          ...
          if (this.indexCode === '3' || this.indexCode === '4') {
            this.pageView.typeCode = 1
          } else {
            this.pageView.typeCode = 2
          }
      ...
      getCount () {
        ...
        this.ajaxPost(this.urlCountElse, (res) => {
      ...
      tabClick (res) {
        ...
        if (this.indexCode === '0' || this.indexCode === '1' || this.indexCode === '2') {
          ...
          this.searchDataInfo = this.searchDataElse
        }
        switch (res.index) {
          case '0':
            this.dataHeaderData = this.dataHeader
