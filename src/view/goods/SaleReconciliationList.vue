...
<script>
  export default {
    data () {
      return {
        uploadData: {
          url: this.$Api.contractGoods.importSaleReconiliationGoods,
          listType: 'text',
          showFileList: false,
          name: 'file',
          data: {},
        ...
        tableHeader: [
          ...
          {label: '梁场采购单号', prop: 'purchaseNumber', slot: true},
          {label: '库别', prop: 'goodsClassName'},
          {label: '送货时间', prop: 'deliveryTimeStr'}
        ],
        page: {
          totalNum: 0,
          currentPage: 1,
          offset: 0,
          refs: 'tab_sale_reconiliation_material_list',
          typeCode: 2
        },
        tablePerformance: 'tab_sale_reconiliation_material_list',
        dataSearch: {},
    ...
    methods: {
      ...
      downLoad () {
        location.href = this.$Api.contractGoods.downloadSalePurchaseTemplate
      },
      searchDataClick (res) {
        this.dataSearch = res
