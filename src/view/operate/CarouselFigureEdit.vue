...
<script>
  export default {
    ...
    methods: {
      getData (id) {
        this.$Ajax.post({
          url: this.$Api.carousel.getDataById,
          data: {id: id},
          cb: res => {
            if (res.data.result) {
              let fr = res.data.data
              this.dataForm.from[0].value = fr.typeCode
              this.dataForm.from[1].value = fr.title
              this.dataForm.from[2].value = fr.url
              this.dataForm.from[3].value = fr.sortOrderNo
              this.dataForm.from[4].value = fr.description
              if (fr.startTimeStr) {
                this.dataForm.from[5].value = [new Date(fr.startTimeStr), new Date(fr.endTimeStr)]
              }
              this.dataForm.from[6].value = fr.imgUrl
              this.imgSrc = this.$ImgPath.imgApi.imgApi + '/' + fr.imgUrl
            }
          }
        })
      },
