  ...
  <div>
    ...
    <el-row style="" class="box_board">
      <el-col class="box_icon_bodrd_border" :span="8">
        <div class="">
          <div class="box_icon_bodrd_text">
            {{ formatData(profitDetailsData.totalMoney) }}
          </div>
        </div>
      ...
      <el-col class="box_icon_bodrd_border" :span="8">
        <div class="">
          <div class="box_icon_bodrd_text">
            {{ formatData(profitDetailsData.totalCost) }}
          </div>
        </div>
      ...
      <el-col class="box_icon_bodrd_border" :span="8">
        <div class="">
          <div class="box_icon_bodrd_text">
            {{ formatData(profitDetailsData.grossProfit) }}
          </div>
        </div>
    ...
    <div class="clearfix">
      <div class="btn_search_time pull-left">
        ...
        <el-radio-group v-model="radioTime" @change="changeTime">
          <el-radio-button
            v-for="(item, index) in dateTimeRadio"
            :key="index"
            :label="item.label"
          ></el-radio-button>
        </el-radio-group>
      ...
      <el-button class="pull-right" type="primary" @click="toPageBoard"
        >梁场概述</el-button
      >
    </div>
    <table-list
      :data-header="profitDetailsHeader"
      :refs="profitDetailsRefs"
      :url="urlProfitDetails"
...
export default {
  ...
  methods: {
    ...
    pageUpdateTable() {
      this.$Utils.pageUpdateTable(
        [[], this.pageProfitDetails],
        this.profitDetailsRefs,
        this
      );
    },
    changePicker(res) {
      ...
      let startTime = this.$Date.dateFormat(
        res[0],
        this.$Date.ISO8601_FORMAT_WITHOUT_TIME
      );
      let endTime = this.$Date.dateFormat(
        res[1],
        this.$Date.ISO8601_FORMAT_WITHOUT_TIME
      );
    ...
    changeTime(res) {
      ...
      let date = new Date();
      let dateNow = this.$Date.dateFormat(
        date,
        this.$Date.ISO8601_FORMAT_WITHOUT_TIME
      );
      let dateCurrentMonth = this.$Date.dateFormat(
    ...
    summaryMethod(param) {
      ...
      if (data.length > 0) {
        columns.forEach((column, index) => {
          if (
            column.property === "materialCount" ||
            column.property === "supplierCount" ||
            column.property === "totalMoney" ||
            column.property === "totalCost" ||
            column.property === "grossProfit" ||
            column.property === "grossProfitRate"
          ) {
            const values = data.map(item => Number(item[column.property]));
            if (!values.every(value => isNaN(value))) {
              sums[index] = values.reduce((prev, curr) => {
                // console.log(prev,curr)
                const curr_ = Number(curr);
                const prev_ = Number(prev);
                if (!isNaN(curr_)) {
                  return (prev_ + curr_).toFixed(4);
                } else {
                  return prev;
                }
              }, 0);
    ...
    toGoodsDetails(res) {
      let routeData = this.$router.resolve({
        ...
        params: {
          state: "detail",
          id: this.pageProfitDetails.projectId,
          date: res.reportDate,
...
.box_icon_bodrd_border {
  height: 140px;
  border-right: 1px solid #bbbbbb;
}
.box_icon_bodrd_border:last-child {
  border-right: none;
}
.box_icon_bodrd_text {
  font-size: 18px;
...
.box_icon_bodrd_text.text_other {
  color: #444444;
  margin-top: 40px;
}
</style>
