  ...
  <div class="p_d_detail">
    <div v-if="dataDetails.length > 0">
      <div v-for="(item, index) in dataDetails">
        ...
        <div class="btn_box">
          <tab-but v-if="item.stateCode === 1 || item.stateCode === 3" :data="btnData" @on-add="addCheck(item.id)" @on-export="exportPurAndDeliver(item.id)" @on-return="applyReturn(item)" @on-exchange="applyExchange(item)"></tab-but>
          <tab-but v-else :data="btnData1" @on-add="addCheck(item.id)" @on-export="exportPurAndDeliver(item.id)"></tab-but>
