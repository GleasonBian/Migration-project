...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      getCheckPass () {
        return (rule, value, callback) => {
          console.log(value !== this.editUserForm.from[1].value)
          if (!value) {
    ...
    mounted () {
      this.$store.state.heightH = this.$refs.header.clientHeight + 2
    }
  }
</script>